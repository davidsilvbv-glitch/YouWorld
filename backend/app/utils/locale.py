...
def get_locale() -> str:
    """Get current locale. Default is 'es' (Spanish).

    Priority: Accept-Language header > thread-local > 'es'
    """
...
def t(key: str, **kwargs) -> str:
    ...
    Falls back to 'es' (Spanish) if the key is not found in the current locale,
    then returns the key itself if not found anywhere.
    """
    locale = get_locale()
...
def get_language_instruction() -> str:
    """Get LLM instruction for the current locale."""
    locale = get_locale()
